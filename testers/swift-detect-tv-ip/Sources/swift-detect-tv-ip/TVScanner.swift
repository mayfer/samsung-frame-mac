    ...
    private func finishScan() {
        ...
        for item in sorted {
            if let existing = uniqueByIP[item.ipAddress] {
                // Prefer friendlier names over UUID-based service names.
                ...
                if existingLooksUUID && !itemLooksUUID {
                    ...
                    // Prefer records that include MAC when multiple service records map to one IP.
                    uniqueByIP[item.ipAddress] = item
                }
            } else {
                uniqueByIP[item.ipAddress] = item
